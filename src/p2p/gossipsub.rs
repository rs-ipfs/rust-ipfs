...
use libp2p::floodsub::{Gossipsub, GossipsubConfig, GossipsubEvent, GossipsubMessage, Topic};
use libp2p::swarm::{NetworkBehaviour, NetworkBehaviourAction, PollParameters, ProtocolsHandler};

/// Currently a thin wrapper around Gossipsub, perhaps supporting both Gossipsub and Gossipsub later.
...
/// Adaptation hopefully supporting both Gossipsub and Gossipsub Messages in the future
...
impl Pubsub {
    /// Delegates the `peer_id` over to [`Gossipsub::new`] and internally only does accounting on
    /// top of the floodsub.
    pub fn new(peer_id: PeerId) -> Self {
        let (tx, rx) = channel::unbounded();
        let mut config = GossipsubConfig::new(peer_id);
        config.subscribe_local_messages = true;
        Pubsub {
            streams: HashMap::new(),
            peers: HashMap::new(),
            floodsub: Gossipsub::from_config(config),
    ...
    /// See [`Gossipsub::add_node_from_partial_view`]
    pub fn add_node_to_partial_view(&mut self, peer_id: PeerId) {
        self.floodsub.add_node_to_partial_view(peer_id);
    }

    /// See [`Gossipsub::remove_node_from_partial_view`]
    ...
    fn poll(
        ...
        loop {
            match futures::ready!(self.floodsub.poll(ctx, poll)) {
                NetworkBehaviourAction::GenerateEvent(GossipsubEvent::Message(msg)) => {
