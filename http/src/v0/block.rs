...
use warp::{http::Response, multipart, path, query, reply, Filter, Rejection, Reply};
...
async fn get_query(ipfs: Ipfs, query: GetQuery) -> Result<impl Reply, Rejection> {
...
async fn put_query(
    ...
    ipfs.put_block(cid, data).await.map_err(StringError::from)?;
...
async fn rm_query(ipfs: Ipfs, query: RmQuery) -> Result<impl Reply, Rejection> {
